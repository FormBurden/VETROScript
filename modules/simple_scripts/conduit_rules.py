...
def emit_conduit_logs(emit_info: bool = True) -> None:
    ...
    Path chain rules (instead of coordinate breadcrumbs):
      • Use the same end-to-end flow as the Distribution/NAP walker, in practice by
        ordering NAPs according to walker order when available, and otherwise by
        along-conduit vertex order.
      • Tokens included on the chain:
          - Distribution lines "on" the conduit:  <Distribution ID> / <Conduit Vetro ID>
          - Vaults on the conduit:                 <Vault Vetro ID>
          - NAPs on the conduit:                   <NAP ID (label)>
      • When a Vault and NAP share the same point, join with " / " (e.g., "VetroID / NAPID").
      • Use " > " when the *distribution line changes*.
      • Within an unbroken distribution, separate the successive Vault/NAP points using " -- ".
      • Start the chain at the **first Vault** encountered on the conduit if one exists.

    Wrapping:
      • The Path column is wrapped to lines of up to 250 visible characters.
      • Wrap can only occur at: " > ", " -- ", " / ".
      • Continuation lines repeat all non-Path columns (aligned), so the block reads vertically.

    This function only logs; it does not modify the underlying results used for Excel.
    """
    import json
    import logging
    ...
    # ---------------------------------------------------------
    def _data_dir() -> Path:
        # Always use project-wide DATA_DIR
    ...
    def _collect_conduits() -> list[dict]:
        # Existing internal loader already used elsewhere in this module
    ...
    def _nap_order_from_walker() -> dict[str, int]:
        """
        Build a first-appearance order index for NAP labels by parsing
        the Distribution/NAP walker paths when available.
        """
        order: dict[str, int] = {}
        idx = 0
        try:
    ...
    def _build_conduit_named_path(conduit: dict) -> str:
        ...
        for seg in segs:
            for pt in (seg or []):
                ...
                v_i += 1
            # do not duplicate between segments; index just keeps increasing
        ...
        for vi, vxy in verts:
            ...
            # Collocate Vault/NAP at the same vertex → " / "
    ...
    def _wrap_chain(chain: str, width: int = 250) -> list[str]:
        """
        Wrap a chain on boundaries ' > ', ' -- ', ' / ' so that each line
        is <= width visible characters. Returns list of lines.
        """
        if not chain:
            return [""]
        ...
        pat = re.compile(r"( > | -- | / )")
        tokens = pat.split(chain)
        # tokens like [chunk, sep, chunk, sep, chunk, ...]
        if tokens:
            first = tokens[0]
            parts.append(("", first))
            i = 1
