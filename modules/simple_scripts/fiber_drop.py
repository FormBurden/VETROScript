...
# def find_color_mismatches(emit_info: bool = True) -> list[str]:
#     """
#     [Drop Issues] Color mismatches — same deep-walk ordering as
...
#     Behavior:
#       • Logs one line per SL in order (✅ for match, ❌ for mismatch).
#       • Adds a bottom “errors-only” recap block with header and end-header.
#     """
#     import logging
#     from modules.hard_scripts.distribution_walker import get_walk_order_index_map
#     logger = logging.getLogger(__name__)

#     # Optional: path trailer lookup (used in per-line logs and recap)
#     try:
#         from modules.hard_scripts.distribution_walker import get_walk_paths_map
#         _paths = get_walk_paths_map()
#     except Exception:
...
#     # Sort rows exactly like the svc-attr check (walker order, then by NAP)
#     def _nap_for_sid(sid: str) -> str:
#         from_path = _extract_nap_id_from_path(_paths.get(sid, ""))
#         if from_path:
#             return from_path
...
#     mismatches: list[str] = []
#     recap_lines: list[str] = []  # for the bottom errors-only recap
#     next_ordinal = (max(order_map.values()) if order_map else 0) + 1
...
#         # Per-line output (match at INFO, mismatch at ERROR)
#         if do_info:
#             if is_match:
#                 logger.info(f"[Drop Issues] ✅ SL # {sl_num}: {sid} — drop={drop_c}; splice={splice_txt}{path_part}")
#             else:
...
#         # Collect mismatches for recap/return
#         if not is_match:
#             mismatches.append(sid)
#             # Compact table-style row for bottom recap
#             cols = [
#                 str(sl_num),
#                 sid,
#                 (drop_c or "(none)"),
...
#             ]
#             if show_path:
#                 cols.append(_paths.get(sid, ""))
#             recap_lines.append(" | ".join(cols))

#     # Bottom recap (errors-only), always present when there are issues
#     if recap_lines:
#         headers = ["SL #", "Service Location ID", "Drop Color", "SL Colors"]
#         if show_path:
#             headers.append("Path")

#         # This banner stays ERROR-level (to match your other sections).
#         log_issue_header(
#             "[Drop Issues] Color Mismatches",
#             [" | ".join(headers)] + recap_lines,
#             logger=logger
#         )
